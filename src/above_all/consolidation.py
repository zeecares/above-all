...
from .notes import index_note, parse_note, set_note_status
...
def _active(db: sqlite3.Connection) -> list:
    return db.execute(
        "SELECT id,title,body,path FROM notes WHERE status='active' ORDER BY id"
    ).fetchall()
...
def _classify(note, active: list) -> dict:
    ...
    overlap = [
        row[0]
        for row in active
        if (tokens := set(_claim(row[2]).split()))
        and len(words & tokens) / max(1, min(len(words), len(tokens))) >= 0.6
    ]
    contradiction = bool(overlap) and any(
        token in words for token in ("not", "never", "instead", "changed", "no")
    )
    if contradiction:
...
def propose_weekly(
    ...
    for record in _candidate_records(scope_dir)[:max_candidates]:
        ...
        change = {
            ...
            "candidate_sha256": _sha(record["path"]),
            **_classify(note, active),
        }
        ...
        if "active_ids" in change:
            change["active_snapshots"] = {
                row[0]: _sha(Path(row[3])) for row in active if row[0] in change["active_ids"]
            }
...
def _validate(payload: dict, scope_dir: Path) -> list[dict]:
    ...
    if (
        ...
        or payload.get("requires_explicit_approve") is not True
    ...
    for change in payload["changes"]:
        ...
        if change["action"] == "trace_candidate":
            ...
            sources = [
                x
                for x in payload.get("trace_analysis_inputs", [])
                if x.get("path", "").endswith(change.get("queue", "") + ".json")
            ]
            if not sources:
            ...
            if (
                not source.is_file()
                or _sha(source) != change.get("source_sha256")
                or _sha(source) != sources[0].get("sha256")
            ):
...
def apply_changeset(
    db: sqlite3.Connection, scope_dir: Path, path: Path, approve: bool = False
) -> dict:
    if not approve:
    ...
    if drifted:
        for cid in sorted(set(drifted)):
            set_note_status(
                scope_dir / "candidates" / f"{cid}.md",
                "needs-review",
                {"review_reason": "active-memory drift after proposal"},
            )
        payload.update(
            status="drifted",
            drifted_candidates=sorted(set(drifted)),
    ...
    try:
        for change in memory:
            ...
            if action == "promote":
                ...
                promote_candidate(db, scope_dir, cid)
                applied.append(change)
            elif action == "replace":
                set_note_status(candidate, "candidate")
                promote_candidate(db, scope_dir, cid, replaces=change["active_id"])
                applied.append(change)
            elif action == "reject_duplicate":
                set_note_status(
                    candidate, "rejected", {"duplicates": f"note:{change['active_id']}"}
                )
                applied.append(change)
                ...
                set_note_status(
                    candidate,
                    "needs-review",
                    {"contradicts": [f"note:{x}" for x in change["active_ids"]]},
                )
                applied.append(change)
        payload.update(
            status="applied",
        ...
        payload.update(
            status="failed", failed_at=datetime.now(timezone.utc).isoformat(), error=str(exc)
        )
        path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
def _event_count(db: sqlite3.Connection, kinds: tuple[str, ...], since: str | None = None) -> int:
    ...
    return db.execute(
        f"SELECT COUNT(*) FROM events WHERE kind IN ({marks}){suffix}", params
    ).fetchone()[0]


def pollution_metrics(db: sqlite3.Connection, scope_dir: Path) -> dict:
    statuses = (
        dict(db.execute("SELECT status,COUNT(*) FROM notes GROUP BY status").fetchall())
        if _table_exists(db, "notes")
        else {}
    )
    sessions = db.execute(
        "SELECT COALESCE(SUM(tokens_in),0),COALESCE(SUM(tokens_out),0),COALESCE(SUM(cost_usd),0),COUNT(*),SUM(CASE WHEN status='done' THEN 1 ELSE 0 END) FROM sessions LEFT JOIN outcomes ON sessions.outcome_id=outcomes.id"
    ).fetchone()
    ...
    failures = _event_count(
        db, ("candidate_creation_failed", "extraction_failure", "empty_extraction_batch")
    )
    if _table_exists(db, "trace_imports"):
        failures += db.execute(
            "SELECT COUNT(*) FROM trace_imports WHERE status='failed'"
        ).fetchone()[0]
    ...
    return {
        ...
        "admission": {
            "candidates_created": _event_count(db, ("candidate_created",)),
            "approved": _event_count(db, ("candidate_approved",)),
            "later_retrieved": retrievals,
        },
        "recent_replacements_or_contradictions": _event_count(
            db, ("note_replaced", "note_contradiction"), since_30d
        ),
        "retrieval_recall": {
            "retrieved": retrievals,
            "ignored_or_corrected": ignored,
            "rate": None if retrievals == 0 else (retrievals - ignored) / retrievals,
        },
        "extraction": {"failures_or_empty_batches": failures, "visible": True},
        "candidate_files": len(list((scope_dir / "candidates").glob("*.md")))
        if (scope_dir / "candidates").is_dir()
        else 0,
        "context_bytes": context.stat().st_size if context.is_file() else 0,
        "prompt_memory_tokens_per_completed_outcome": None
        if not sessions[4]
        else sessions[0] / sessions[4],
        "tokens_in": sessions[0],
        "tokens_out": sessions[1],
        "cost_usd": sessions[2],
        "sessions": sessions[3],
    }
