...
def run_due(db: sqlite3.Connection, now: datetime | None = None) -> dict:
    ...
    for watch in due_watches(db, clock.isoformat()):
        ...
        event_id = fire_watch(db, watch["id"], {"trigger": "due", "spec": spec}, spec.get("value"), clock)
        fired.append({"watch_id": watch["id"], "event_id": event_id})
    return {"fired": fired, "gate": value_gate(db)}


def run_maintenance(db: sqlite3.Connection, scope_dir: Path, weekly: bool = False, max_candidates: int = 100) -> dict:
    result = {"expiry": daily_expiry_sweep(db, scope_dir), "watches": run_due(db), "pollution": pollution_metrics(db, scope_dir)}
    if weekly:
        path = propose_weekly(db, scope_dir, scope_dir / "changesets", max_candidates=max_candidates)
