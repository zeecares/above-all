...
def reviewed(path):
    path.write_text(path.read_text().replace("status: candidate", "status: reviewed"))
    return path
...
def test_daily_expiry_marks_needs_review_and_never_touches_candidate(tmp_path):
    ...
    old = note(root / "notes" / "old.md", "old fact", stale_after="2026-01-01")
    index_note(db, old)
    boundary = note(root / "notes" / "boundary.md", "today fact", stale_after="2026-09-18")
    index_note(db, boundary)
    future = note(root / "notes" / "future.md", "future fact", stale_after="2027-01-01")
    index_note(db, future)
    candidate = create_candidate(
        root / "candidates", "Maybe", "candidate", "fact", ["trace:t1"], stale_after="2026-01-01"
    )
...
def test_daily_expiry_missing_file_fails_before_any_mutation(tmp_path):
    root, db = scope(tmp_path)
    a = note(root / "notes" / "a.md", "a", stale_after="2026-01-01")
    index_note(db, a)
    b = note(root / "notes" / "b.md", "b", stale_after="2026-01-01")
    index_note(db, b)
    b.unlink()
    with pytest.raises(FileNotFoundError):
        daily_expiry_sweep(db, root, date(2026, 9, 18))
...
def test_weekly_is_bounded_and_apply_requires_explicit_approve(tmp_path):
    root, db = scope(tmp_path)
    active = note(root / "notes" / "a.md", "alpha beta")
    index_note(db, active)
    duplicate = create_candidate(root / "candidates", "Dup", "alpha beta", "fact", ["trace:t1"])
    new = reviewed(
        create_candidate(root / "candidates", "New", "gamma delta", "fact", ["trace:t2"])
    )
...
def test_apply_revalidates_and_returns_drifted_candidate_to_review(tmp_path):
    root, db = scope(tmp_path)
    candidate = reviewed(
        create_candidate(root / "candidates", "New", "same claim", "fact", ["trace:t1"])
    )
    changeset = propose_weekly(db, root, tmp_path / "changes")
    other = create_candidate(root / "candidates", "Other", "same claim", "fact", ["trace:t2"])
    approve_candidate(db, root, other.stem)
    result = apply_changeset(db, root, changeset, approve=True)
...
def test_changeset_rejects_unknown_or_tampered_action_before_writes(tmp_path):
    root, db = scope(tmp_path)
    candidate = reviewed(
        create_candidate(root / "candidates", "New", "gamma", "fact", ["trace:t1"])
    )
    changeset = propose_weekly(db, root, tmp_path / "changes")
    payload = json.loads(changeset.read_text())
    payload["changes"].append({"action": "erase_everything"})
    changeset.write_text(json.dumps(payload))
    with pytest.raises(ValueError, match="unknown"):
        apply_changeset(db, root, changeset, approve=True)
...
def test_replace_supersedes_old_and_preserves_evidence(tmp_path):
    root, db = scope(tmp_path)
    active = note(root / "notes" / "a.md", "deploy production on friday", sources="[trace:old]")
    index_note(db, active)
    candidate = reviewed(
        create_candidate(
            root / "candidates",
            "Changed",
            "do not deploy production on friday",
            "procedure",
            ["trace:new"],
        )
    )
    ...
    old = parse_note(active.read_text())
    new = parse_note((root / "notes" / f"{candidate.stem}.md").read_text())
...
def test_candidate_contradiction_stays_for_review_and_preserves_active(tmp_path):
    root, db = scope(tmp_path)
    active = note(root / "notes" / "a.md", "deploy production on friday")
    index_note(db, active)
    candidate = create_candidate(
        root / "candidates",
        "Changed",
...
def test_trace_queues_are_bounded_ingested_proposals_and_malformed_fails(tmp_path):
    ...
    traces = [x for x in payload["changes"] if x["action"] == "trace_candidate"]
    assert (
        len(traces) == 1
        and traces[0]["proposal"] == item
        and payload["trace_analysis_inputs"][0]["items"] == 2
    )
    assert not list((root / "candidates").iterdir())
    (analysis / "bad-candidates.json").write_text("{")
    with pytest.raises(ValueError, match="malformed trace queue"):
        propose_weekly(db, root, tmp_path / "changes2", analysis)
...
def test_watch_identity_is_unique_idempotent_and_persists(tmp_path):
    ...
    db = migrate(path, GLOBAL_MIGRATIONS)
    assert (
        db.execute("SELECT next_fire_at FROM watches WHERE id=?", (one,)).fetchone()[0]
...
def test_value_gate_fails_closed_honors_policy_and_accepts_only_categories(tmp_path):
    ...
    internal = create_watch(
        db, "clock", {"at": "internal"}, "2026-09-18T09:00:00+00:00", policy="internal-only"
    )
    ...
    decisions = iter(
        [
            {"category": "risk", "reason": "avoids missed deadline"},
            {"category": "bogus", "reason": "hello"},
        ]
    )
...
def test_cadence_reschedules_and_invalid_fails(tmp_path):
    ...
    assert (
        db.execute("SELECT next_fire_at FROM watches WHERE id=?", (cadence,))
        .fetchone()[0]
        .startswith("2026-09-18T11:00:00")
    )
    with pytest.raises(ValueError, match="positive integer"):
        create_watch(db, "cadence", {"minutes": 0})


def test_pollution_metrics_expose_all_tripwire_groups(tmp_path):
    root, db = scope(tmp_path)
    db.execute(
        "INSERT INTO events(ts,kind,payload_json) VALUES ('2026-09-18','note_retrieved','{}'),('2026-09-18','retrieval_corrected','{}'),('2026-09-18','empty_extraction_batch','{}')"
    )
    metrics = pollution_metrics(db, root)
    assert {
        "admission",
        "recent_replacements_or_contradictions",
        "retrieval_recall",
        "extraction",
        "prompt_memory_tokens_per_completed_outcome",
    } <= metrics.keys()
...
def test_due_fire_is_compare_and_swap_idempotent_and_cadence_does_not_drift(tmp_path):
    from above_all.operations import run_due

    path = tmp_path / "global.db"
    first = migrate(path, GLOBAL_MIGRATIONS)
    second = migrate(path, GLOBAL_MIGRATIONS)
    watch = create_watch(first, "cadence", {"minutes": 60}, "2026-09-18T09:00:00+00:00")
    due = due_watches(first, "2026-09-18T10:30:00+00:00")[0]
    event = fire_watch(
        first,
        watch,
        {},
        now=datetime(2026, 9, 18, 10, 30, tzinfo=timezone.utc),
        expected_next_fire_at=due["next_fire_at"],
    )
    duplicate = fire_watch(
        second,
        watch,
        {},
        now=datetime(2026, 9, 18, 10, 30, tzinfo=timezone.utc),
    ...
    assert event is not None and duplicate is None
    assert first.execute("SELECT COUNT(*) FROM proactive_events").fetchone()[0] == 1
    assert (
        first.execute("SELECT next_fire_at FROM watches WHERE id=?", (watch,)).fetchone()[0]
        == "2026-09-18T11:00:00+00:00"
    )
    assert run_due(first, datetime(2026, 9, 18, 10, 30, tzinfo=timezone.utc))["fired"] == []
